...
export async function POST(req: Request) {
  ...
  if (await hasActiveChatStreamsInSession(sessionId)) {
    return Response.json(
      {
        error:
          "Wait for the active agent run in this session to finish before generating a PR.",
      },
      { status: 409 },
    );
  }
    ...
    return await withSessionGitMutation(
      {
        ...
        sandbox,
        busyMessage:
          "Another git write is already running for this session. Wait for it to finish before generating a PR.",
      },
      async () => {
...
Respond with ONLY the commit message, nothing else.`,
          ...
          const postCommitHead = await sandbox.exec(
            "git rev-parse HEAD",
            cwd,
            5000,
          );
        ...
        if (upstreamRef.startsWith("fork/")) {
          ...
          const forkOwner = extractGitHubOwnerFromRemoteUrl(
            forkUrlResult.stdout,
          );
          if (forkOwner) {
        ...
        if (needsPush) {
          ...
          const branchExistsOnRemote =
            remoteBranchCheck.stdout.trim().length > 0;
