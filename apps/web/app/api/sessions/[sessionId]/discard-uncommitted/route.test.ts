...
const hasActiveChatStreamsInSessionMock = mock(async () => false);
const withSessionGitMutationMock = mock(
  async (_params: unknown, fn: () => Promise<Response>) => await fn(),
);
...
const execMock = mock(async (command: string) => {
  ...
  if (command === "git reset --hard HEAD") {
    return successResult();
  }
...
mock.module("@/lib/db/sessions", () => ({
  hasActiveChatStreamsInSession: hasActiveChatStreamsInSessionMock,
}));

mock.module("@/lib/git/session-git-mutation", () => ({
  SessionGitMutationBusyError,
  withSessionGitMutation: withSessionGitMutationMock,
}));
...
describe("/api/sessions/[sessionId]/discard-uncommitted", () => {
  beforeEach(() => {
    ...
    hasActiveChatStreamsInSessionMock.mockClear();
    hasActiveChatStreamsInSessionMock.mockImplementation(async () => false);
    withSessionGitMutationMock.mockClear();
    withSessionGitMutationMock.mockImplementation(
      async (_params: unknown, fn: () => Promise<Response>) => await fn(),
    );
  ...
  test("discards all uncommitted changes when no file is specified", async () => {
    const { POST } = await loadRouteModule();
  ...
  test("returns 409 when a chat in the session is still streaming", async () => {
    hasActiveChatStreamsInSessionMock.mockImplementationOnce(async () => true);
    const { POST } = await loadRouteModule();
    ...
    expect(await response.json()).toEqual({
      error:
        "Wait for the active agent run in this session to finish before discarding changes.",
    });
    expect(connectSandboxMock).not.toHaveBeenCalled();
  });

  test("returns 409 when another session git write is already running", async () => {
    withSessionGitMutationMock.mockImplementationOnce(async () => {
      throw new SessionGitMutationBusyError(
        "Another git write is already running for this session. Wait for it to finish before discarding changes.",
      );
    });
    const { POST } = await loadRouteModule();
