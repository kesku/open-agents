...
export async function runCreateRepoWorkflow({
  ...
  return withSessionGitMutation(
    {
      sessionId,
      leaseType: "create-repo",
      sandbox: {
        exec: sandbox.exec,
        workingDirectory: cwd,
      },
      busyMessage:
        "Another git write is already running for this session. Wait for it to finish before creating a repository.",
    },
    async () => {
...
Respond with ONLY the commit message, nothing else.`,
      ...
      if (!pushResult.success) {
        ...
        return {
          ok: false,
          response: repoCreatedError(
            `Failed to push: ${pushOutput.slice(0, 100)}`,
          ),
        };
      }
